# Network analysis of high-density microelectrode recordings
## Figures

[Generating all figures](all_figures.py)

### Main figures
1. [Example for segmentation of an axon](figure_axon.py)
2. [Comparison of axon segmentation methods with groundtruth](figure_groundtruth.py)
3. [Comparison of axon segmentation methods by receiver operator characteristics](figure_comparison.py)
4. [Example for segmentation of a dendrite](figure_dendrite.py)
5. [Estimation of structural and functional connectivity](figure_structural_and_functional.py)
6. [Comparison of structural and functional connectivity, putative chemical synapses](figure_structur_vs_function_and_synapses.py)
7. [Polychronous groups](figure_polychronous_groups.py)

### Supplementary figures
1. [Summary for all cultures](figure_multiple_networks.py)
1. [Segmentation of all neurons](figure_all_neurons.py)
2. [Parameter dependency of structural and functional connectivity](figure_more_structure_vs_function.py)

### Supplementary movies
* [Movies from spike-triggered averages](all_animations.py)

### Previous figures 
These figures are not automatically generated:
* [Estimation of structural connectivity](figure_structural.py)
* [Estimation of functional connectivity](figure_functional.py)
* [Comparison of structural und functional connectivity](figure_structur_vs_function.py)
* [Putative chemical synapses](figure_synapse.py)
